...
from cortex_DIM_DEMI_demi.functions.dim_losses import donsker_varadhan_loss, infonce_loss, fenchel_dual_loss
from cortex_DIM_DEMI_demi.functions.dim_losses import demi_ce_loss, demi_mi_loss
from cortex_DIM_DEMI_demi.nn_modules.convnet import Convnet
from cortex_DIM_DEMI_demi.nn_modules.resnet import ResNet
from cortex_DIM_DEMI_demi.nn_modules.mi_networks import MI1x1ConvNet, NopNet
...
def compute_dim_loss(l_enc, m_enc, measure, mode):
    ...
    elif mode == 'demi' and measure == 'ce':
        loss = demi_ce_loss(l_enc, m_enc)
    elif mode == 'demi' and measure == 'mi':
        loss = demi_mi_loss(l_enc, m_enc)
    else:
...
class LocalDIM(ModelPlugin):
    ...
    def routine(self, outs=None, measure='JSD', mode='fd', scale=1.0, act_penalty=0.):
        ...
        #print('DIM:{}, {}'.format(L[0,0,0], G[0,0]))
...
class LocalDEMI(ModelPlugin):
    ...
    def build(self, local_MINet, global_MINet):
        '''

        Args:
            local_MINet: local_MINet
            global_MINet: global_MINet

        '''
        self.nets.local_MINet = local_MINet
        self.nets.global_MINet = global_MINet

    def routine(self, outs=None, name='local'):
        '''
        ...
        loss = compute_dim_loss(L, G, measure='ce', mode='demi')

        self.add_losses(local_MINet=loss)
        self.add_losses(global_MINet=loss)
